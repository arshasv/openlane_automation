...
if [ ! -d "/app/openlane2" ]; then
    git clone https://github.com/The-OpenROAD-Project/OpenLane.git /app/openlane2
    chown -R $(whoami):$(whoami) /app/openlane2
fi
...
cp designs/spm/config.tcl designs/spm/pin_order.cfg designs/"$NEW_FOLDER"
...
nix-shell -p --command "openlane designs/$NEW_FOLDER/config.tcl"
