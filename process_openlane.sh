...
generate_openroad_files() {
...

    {
        ...
        echo "#set ::env(DESIGN_IS_CORE) 0"
        echo "set ::env(LEC_ENABLE) 0"
        echo "##################################################################"
        echo "# LINTING"
        echo "##################################################################"
        echo "set ::env(RUN_LINTER) 1"
        echo "set ::env(QUIT_ON_LINTER_WARNINGS) 0"
        echo "set ::env(QUIT_ON_LINTER_ERRORS) 1"
        echo "##################################################################"
        echo "# SYNSTHESIS"
        echo "##################################################################"
        echo "set ::env(SYNTH_CLOCK_UNCERTAINTY) 0.25"
        echo "set ::env(SYNTH_CLOCK_UNCERTAINTY) 0.15"
        echo "# DELAY/AREA 0-4/0-3"
        echo "#set ::env(SYNTH_STRATEGY) \"DELAY 3\""
        echo "set ::env(SYNTH_NO_FLAT) 0"
        echo "set ::env(SYNTH_SHARE_RESOURCES) 1"
        echo "set ::env(SYNTH_ADDER_TYPE) \"YOSYS\""
        echo "set ::env(BASE_SDC_FILE) [glob \$::env(DESIGN_DIR)/src/*.sdc]"
        echo "set ::env(SYNTH_FLAT_TOP) 0"
        echo "set ::env(SYNTH_USE_PG_PINS_DEFINES) \"USE_POWER_PINS\""
        echo "set ::env(QUIT_ON_TIMING_VIOLATIONS) 1"
        echo "set ::env(QUIT_ON_SETUP_VIOLATIONS) 1"
        echo "set ::env(QUIT_ON_HOLD_VIOLATIONS) 1"
        echo "##################################################################"
        echo "# FLOORPLAN"
        echo "##################################################################"
        echo "set ::env(RUN_TAP_DECAP_INSERTION) 1"
        echo "set ::env(FP_CORE_UTIL) 45"
        echo "set ::env(FP_ASPECT_RATIO) 1"
        if [[ -n "$die_area" ]]; then
            ...
            echo "set ::env(FP_SIZING) \"relative\""
            echo "# set ::env(FP_SIZING) \"absolute\""
            echo "# set ::env(DIE_AREA) \"0 0 600 600\""
        fi
        echo "set ::env(VDD_NETS) \"vccd1\""
        echo "set ::env(GND_NETS) \"vssd1\""
        echo "set ::env(SYNTH_USE_PG_PINS_DEFINES) \"USE_POWER_PINS\""
        echo "set ::env(FP_PIN_ORDER_CFG) [glob \$::env(DESIGN_DIR)/pin_order.cfg]"
        echo "set ::env(FP_PDN_CORE_RING) 0"
        echo "set ::env(FP_PDN_MULTILAYER) 0"
        echo "set ::env(RT_MAX_LAYER) \"met4\""
        echo "set ::env(FP_PDN_SKIPTRIM) 0"
        echo "set ::env(FP_PDN_ENABLE_RAILS) 1"
        echo "#set ::env(FP_PDN_CORE_RING_VWIDTH) 3.1"
        echo "#set ::env(FP_PDN_CORE_RING_HWIDTH) 3.1"
        echo "#set ::env(FP_PDN_CORE_RING_VOFFSET) 12.45"
        echo "#set ::env(FP_PDN_CORE_RING_HOFFSET) 12.45"
        echo "#set ::env(FP_PDN_CORE_RING_VSPACING) 1.7"
        echo "#set ::env(FP_PDN_CORE_RING_HSPACING) 1.7"
        echo "#set ::env(FP_PDN_VWIDTH) 3.1"
        echo "#set ::env(FP_PDN_HWIDTH) 3.1"
        echo "#set ::env(FP_PDN_VSPACING) [expr::(5 * \$FP_PDN_CORE_RING_VWIDTH)]"
        echo "#set ::env(FP_PDN_HSPACING) [expr::(5 * \$FP_PDN_CORE_RING_HWIDTH)]"
        echo "#set ::env(FP_PDN_VPITCH) 180"
        echo "#set ::env(FP_PDN_HPITCH) 180"
        echo "#set ::env(FP_PDN_VOFFSET) 5"
        echo "#set ::env(FP_PDN_HOFFSET) 5"
        echo "##################################################################"
        echo "# PLACEMENT"
        echo "##################################################################"
        echo "set ::env(PL_TARGET_DENSITY) 0.55"
        echo "set ::env(PL_BASIC_PLACEMENT) 0"
        echo "set ::env(PL_RESIZER_BUFFER_INPUT_PORTS) 1"
        echo "set ::env(PL_RESIZER_DESIGN_OPTIMIZATIONS) 1"
        echo "set ::env(PL_RESIZER_TIMING_OPTIMIZATIONS) 1"
        echo "set ::env(GLB_RESIZER_DESIGN_OPTIMIZATIONS) 1"
        echo "set ::env(GLB_RESIZER_TIMING_OPTIMIZATIONS) 1"
        echo "set ::env(RUN_CTS) 1"
        echo "set ::env(RUN_FILL_INSERTION) 1"
        echo "##################################################################"
        echo "# ROUTING"
        echo "##################################################################"
        echo "set ::env(ROUTING_CORES) 4"
        echo "set ::env(GRT_ALLOW_CONGESTION) 1"
        echo "set ::env(DRT_OPT_ITERS) 20"
        echo "##################################################################"
        echo "# SIGNOFF"
        echo "##################################################################"
        echo "set ::env(RUN_CVC) 1"
        echo "set ::env(RUN_IRDROP_REPORT) 1"
    } > "$config_file"
...
# Run OpenLane flow
nix-shell --command "openlane designs/$design_name/config.tcl" || { echo "OpenLane flow failed"; exit 1; }
